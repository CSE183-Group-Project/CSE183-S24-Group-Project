...
db.define_table('checklists', Field('sampling_event_identifier'),
                Field('latitude'), Field('longitude'), Field('observation_date'), Field('time_observation'),
                Field('observer_id'), Field('duration_minute'))

if db(db.species).isempty():
    with open('apps/location/data/species.csv', 'r') as f:
        reader = csv.reader(f)
        for row in reader:
            db.species.insert(common_name=row[0])

if db(db.sightings).isempty():
    with open('apps/location/data/sightings.csv', 'r') as f:
        reader = csv.reader(f)
        for row in reader:
            db.sightings.insert(sampling_event_identifier=row[0], common_name=row[1], observation_count=row[2])

if db(db.checklists).isempty():
    with open('apps/location/data/checklists.csv', 'r') as f:
        reader = csv.reader(f)
        for row in reader:
            db.checklists.insert(sampling_event_identifier=row[0], latitude=row[1], longitude=row[2],
                                 observation_date=row[3], time_observation=row[4], observer_id=row[5],
